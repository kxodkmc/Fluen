...
export interface ShinyTextProps {
  ...
  text: string;
  /** Disables the shiny effect when set to true. */
  disabled?: boolean;
  ...
  color?: string;
  /** The color of the shine/highlight effect. */
  shineColor?: string;
  /** The angle (in degrees) of the gradient spread. */
  spread?: number;
  /** If true, the animation reverses direction instead of looping. */
  yoyo?: boolean;
  /** Pauses the animation when the user hovers over the text. */
  pauseOnHover?: boolean;
  /** The direction the shine moves across the text. */
  direction?: 'left' | 'right';
  /** Pause duration (in seconds) between animation cycles. */
  delay?: number;
}
</script>
...
<style scoped>
/* Inline-block so the gradient background clips to the text bounding box. */
.shiny-text {
