...
export interface DarkVeilProps {
  /** Shifts the hue of the entire animation (in degrees). */
  hueShift?: number;
  /** Intensity of the noise/grain effect (0–1). */
  noiseIntensity?: number;
  /** Intensity of the scanline effect (0–1). */
  ...
  warpAmount?: number;
  /** Scale factor for the internal render resolution (1 = full resolution). */
...
/* ------------------------------------------------------------------ *
 * Shaders
 * The vertex shader is a trivial pass-through; the fragment shader
 * evaluates a pre-trained CPPN (composition pattern-producing neural
 * network) to generate the flowing dark-veil texture. Hue shift,
 * scanlines, noise and warp are applied as post-processing in main().
 * ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ *
 * State — non-reactive variables that persist across the component
 * lifecycle but don't need to trigger re-renders.
 * ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ *
 * Uniform sync — pushes prop values into shader uniforms whenever
 * they change (avoids restarting the render loop).
 * ------------------------------------------------------------------ */
...
onMounted(() => {
  ...
  // Observe parent size changes for responsive rendering.
...
onUnmounted(() => {
  ...
  if (renderer) {
    const ext = renderer.gl.getExtension('WEBGL_lose_context') as { loseContext: () => void } | null;
    ext?.loseContext();
    renderer = null;
  }
