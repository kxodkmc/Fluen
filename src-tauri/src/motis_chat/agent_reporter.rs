...
//! **内部工具调用**（开始/结束）以及 **Motis 主会话工具的执行结果**
//! 以 Tauri 事件透传给前端：
//!
//! | 方法 | 会话 | 事件 | 触发点 |
//! |------|------|------|--------|
...
//!
//! 上报器实现了 [`ToolEventSink`]，构建 Motis 运行时与子代理联邦时注入
//! （见 `runtime::build_motis_tool_registry` / `federation`），对所有
//! 工具集生效。
...
use super::events::{
    AgentFinishedPayload, AgentStartedPayload, AgentToolCallPayload, AgentToolResultPayload,
    ToolResultPayload, EVENT_AGENT_FINISHED, EVENT_AGENT_STARTED, EVENT_AGENT_TOOL_CALL,
    EVENT_AGENT_TOOL_RESULT, EVENT_PROJECT_UPDATED, EVENT_TOOL_RESULT,
};
...
mod tests {
    ...
    fn tool_events_route_to_parent_delegation() {
        ...
        assert_eq!(events[0].1["agent_id"], "academic_writer");
    ...
    fn delegation_lifecycle_payloads() {
        let (reporter, events, _session) = reporter_with_delegation();
        reporter.delegation_started("parent-call", "academic_writer", "写引言", 600_000);
