...
//! （与知识库 `KnowledgeToolProvider`、论文内容 `PaperContentTool` 相同的注入方式）。
//!
//! ## 支持的操作（`action` 参数）
//!
...
//! - **写操作需用户确认**：write / edit / append 由 motis 运行时注入的
//!   `ToolApprovalExtension` 弹窗征求用户同意后才落盘（read 免审批）。
//!   注意：confluent 内置 `fs_write_file` 会被 motis 装配时过滤（绕过确认）；
//!   `fs_execute_command` 同样纳入审批。
//!
//! ## 定位
//!
...
impl ProjectFileTool {
    ...
    pub fn new(project_path: String) -> Self {
        ...
        Self {
            schema: ToolSchema {
                name: PROJECT_FILE_TOOL_NAME.into(),
                description: "在论文项目内读写、编辑文件：可读取、整体写入（创建/覆盖）、精确替换、追加内容。所有路径相对项目根，禁止越界（.. / 绝对路径 / .git）。注意：write / edit / append 属于写操作，执行前会弹出确认框，需用户点击「应用」后才真正写入文件；read 无需确认。".into(),
                parameters,
            },
    ...
    fn resolve_path(&self, rel: &str, create_parent: bool) -> Result<PathBuf, ToolError> {
        ...
        let parent = joined
            .parent()
            .ok_or_else(|| ToolError::InvalidParams(format!("path 无父目录: {}", rel)))?;
        if create_parent && !parent.exists() {
            std::fs::create_dir_all(parent).map_err(|e| {
                ToolError::ExecutionFailed(format!("创建目录失败 {}: {}", parent.display(), e))
            })?;
        }
        let canon_parent = parent.canonicalize().map_err(|e| {
            ToolError::ExecutionFailed(format!("目录不存在或不可访问 {}: {}", parent.display(), e))
        })?;
        let canon_root = self.project_path.canonicalize().map_err(|e| {
            ToolError::ExecutionFailed(format!("项目根不可访问 {}: {}", self.project_path.display(), e))
        })?;
    ...
    fn read_file_content(&self, full_path: &Path, rel: &str) -> Result<String, ToolError> {
        let metadata = std::fs::metadata(full_path).map_err(|e| {
            ToolError::ExecutionFailed(format!("读取元信息失败 {}: {}", full_path.display(), e))
        })?;
        ...
        std::fs::read_to_string(full_path).map_err(|e| {
            ToolError::ExecutionFailed(format!("读取文件失败 {}: {}", full_path.display(), e))
        })
    }
    ...
    fn write_file(&self, full_path: &Path, content: &str, rel: &str) -> Result<Value, ToolError> {
        ...
        std::fs::write(full_path, content).map_err(|e| {
            ToolError::ExecutionFailed(format!("写入文件失败 {}: {}", full_path.display(), e))
        })?;
        Ok(json!({
            "path": rel,
...
impl Tool for ProjectFileTool {
    ...
    async fn invoke(
        ...
        let path = input
            ...
            .ok_or_else(|| ToolError::InvalidParams("缺少 path 参数".into()))?;
        ...
        match action {
            ...
            "write" => {
                let content = input
                    .get("content")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ToolError::InvalidParams("write 需要 content 参数".into()))?;
            ...
            "append" => {
                let content = input
                    .get("content")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ToolError::InvalidParams("append 需要 content 参数".into()))?;
                let full_path = self.resolve_path(path, true)?;
            ...
            "edit" => {
                let old_string = input
                    .get("old_string")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ToolError::InvalidParams("edit 需要 old_string 参数".into()))?;
                let new_string = input
                    .get("new_string")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ToolError::InvalidParams("edit 需要 new_string 参数".into()))?;
                if old_string.is_empty() {
                    return Err(ToolError::InvalidParams("old_string 不能为空".into()));
                }
                let full_path = self.resolve_path(path, false)?;
                ...
                match count {
                    ...
                    n => Err(ToolError::InvalidParams(format!(
...
/// 包装 [`ProjectFileTool`] 为 [`ToolProvider`]。
pub struct ProjectFileToolProvider {
    tool: Arc<ProjectFileTool>,
}

impl ProjectFileToolProvider {
    /// 构造提供者。
    pub fn new(project_path: String) -> Self {
        Self {
            tool: Arc::new(ProjectFileTool::new(project_path)),
        }
    }
}

#[async_trait]
impl ToolProvider for ProjectFileToolProvider {
...
mod tests {
    ...
    async fn write_creates_file_and_reads_back() {
        ...
        // read 读回
        let read = tool
            .invoke(json!({ "action": "read", "path": "notes/scratch.md" }), &ctx())
            .await
            .unwrap();
    ...
    async fn path_traversal_rejected() {
        ...
        let err = tool
            .invoke(
                json!({ "action": "read", "path": "a/../../secret.txt" }),
                &ctx(),
            )
            .await
            .unwrap_err();
    ...
    async fn protected_git_path_rejected() {
        ...
        let err = tool
            .invoke(
                json!({ "action": "read", "path": ".git/config" }),
                &ctx(),
            )
            .await
            .unwrap_err();
        ...
        let err = tool
            .invoke(
                json!({ "action": "read", "path": ".git/objects/aa/bb" }),
                &ctx(),
            )
            .await
            .unwrap_err();
    ...
    async fn read_missing_file_errors() {
        ...
        let err = tool
            .invoke(json!({ "action": "read", "path": "ghost.md" }), &ctx())
            .await
            .unwrap_err();
    ...
    async fn missing_action_or_path_rejected() {
        ...
        let err = tool.invoke(json!({ "path": "a.txt" }), &ctx()).await.unwrap_err();
        assert!(matches!(err, ToolError::InvalidParams(_)));

        let err = tool.invoke(json!({ "action": "read" }), &ctx()).await.unwrap_err();
        assert!(matches!(err, ToolError::InvalidParams(_)));

        let err = tool
            .invoke(json!({ "action": "bogus", "path": "a.txt" }), &ctx())
            .await
            .unwrap_err();
        assert!(matches!(err, ToolError::InvalidParams(_)));
    }
    ...
    async fn symlink_escape_rejected() {
        ...
        #[cfg(unix)]
        {
            ...
            let err = tool
                .invoke(json!({ "action": "read", "path": "link/secret.txt" }), &ctx())
                .await
                .unwrap_err();
            assert!(matches!(err, ToolError::InvalidParams(_)), "目录符号链接逃逸应被拒绝");

            // 文件符号链接：最终目标检查拦截
            std::os::unix::fs::symlink(outside.join("secret.txt"), dir.join("secret_link.txt"))
                .unwrap();
            let err = tool
                .invoke(json!({ "action": "read", "path": "secret_link.txt" }), &ctx())
                .await
                .unwrap_err();
            assert!(matches!(err, ToolError::InvalidParams(_)), "文件符号链接读取应被拒绝");
            let err = tool
                .invoke(
                ...
                .await
                .unwrap_err();
            assert!(matches!(err, ToolError::InvalidParams(_)), "文件符号链接写入应被拒绝");
        }
        #[cfg(not(unix))]
        {
            ...
            if std::os::windows::fs::symlink_dir(&outside, dir.join("link")).is_ok() {
                let err = tool
                    ...
                    .await
                    .unwrap_err();
                assert!(matches!(err, ToolError::InvalidParams(_)), "符号链接逃逸应被拒绝");
            }
        }
